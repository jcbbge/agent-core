...
**Retirement note (2026-08-12, tower-perf / decisions-w2):** PHASE 1 re-measurement (commit d51b432, `briefs/fringe/done/cord-flywheel.evidence.md`) recommended not building the daemon — fork-per-event cost measured cheap (~17–20 ms bun floor), and ~90% of remaining hook time was one algorithmic wart: full JSONL re-parse + per-row `normCwd()` git calls in `tower-ledger.mjs`. tower-perf shipped the surgical fix instead: incremental byte-offset inbox/board cursors (`~/.tower/cursors/ledger.inbox.cursor.json`, `board.scope.cursor.json`) plus fire-and-forget `report-metadata` in `herdr-task-report.sh`. Post-ship re-measurement (15 runs, flywheel method): stop-guard 290.5→31.0 ms (−89%), prompt-inject 200.2→31.5 ms (−84%), session-start 222.0→43.2 ms (−81%); herdr-task-report tool [HERDR] 43.1→33.1 ms. Daemon premise (abolish the fork) remains valid fringe insight but is **retired from the top-5 queue** — the measured pain is gone without kill-9-eats-events risk. Re-open only if vein hook-time regrows on a future corpus.
