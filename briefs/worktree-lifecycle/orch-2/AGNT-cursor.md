...
You own **`~/cursor-shim` entirely, and nothing else**. Your sibling owns
`~/herdr-spine`. You may read anything; you may write only under
`~/cursor-shim/`.

Your working directory is a git worktree of `~/cursor-shim` created for you. Its
base branch is `feat/a5-batch-record`, not `main` — that is expected. Edit
`cursor-spine` and `cursor-finish` relative to your own cwd, and exercise them as
`"$PWD/cursor-finish" ...` so you are testing your copy, never the installed one
at `~/cursor-shim/cursor-finish`.

Your repo's own gate is `bash docs/qa-verify.sh` (71/71 at last record). Run it
exactly as CI does, from your worktree, and report the count before and after
your changes. Do not run `bun install` or any `prepare` script from the worktree.

**Ordering is binding and safety-critical: 3.3 -> 3.1 -> 2.3.** The preservation
guard must be proven before the trap is armed. Re-read the HAZARD note above
before you touch `cleanup()`.

### Task 3.3 — the preservation guard (do this first)

Preserve the safety property the manual sweep proved necessary: **a blocked
commit must never become data loss.** Change `cleanup()`'s force-remove semantics
so that, for each worktree:

- if the worktree is on a detached HEAD, park it on a branch before anything else
  or the commits are lost;
- if the worktree is dirty, attempt to preserve the work on its branch;
- **re-check** `git status --porcelain` after the attempt. If the tree is still
  dirty — the pre-commit hook refused, which `credential-guard` legitimately
  does — **keep the directory**, emit a clear report naming the path, the branch,
  and the count of unpreserved changes, and do not remove and do not
  `git branch -D`;
- only when the work is safely on a branch (or there was never any work) may the
  worktree be removed and its branch deleted.

`cleanup()` must remain idempotent, because after task 3.1 it will run both at
`:374` and via the trap. Prove idempotence explicitly: calling it twice must not
error and must not change the second-call outcome.

- **Done when:** a test against a real throwaway repo, with a real pre-commit
  hook that refuses, shows the worktree **kept** and a clear report emitted;
  and the same test with a permissive hook shows the work preserved on the branch
  and the worktree removed. Quote both runs, and quote a double-invocation
  proving idempotence.

### Task 3.1 — arm the trap

`cursor-finish` — install `trap cleanup EXIT` so a halted, failed, or crashed run
reaps as reliably as a successful one. Note the file is `set -euo pipefail`
(`:4`) and `die()` exits 1 (`:15`), so an EXIT trap catches every `die` in
`wait_workers`, `integrate`, `triage`, and `land_main`, plus any unexpected
crash. Confirm the double-invocation (`:374` plus the trap) is harmless — that is
what 3.3's idempotence proof buys you. Consider whether the trap should preserve
and report the failing exit status rather than mask it; if `cleanup` can change
`$?`, say so and handle it.

**Do not arm this before 3.3 is proven.**

- **Done when:** a deliberately failed run (force `qa-verify` red) leaves zero
  worktrees behind, demonstrated with `git worktree list` **before and after**;
  and a failed run whose work cannot be preserved leaves the directory standing
  with a report. Quote both, and quote the failing run's exit status.

### Task 2.3 — sparse checkout on the cursor path

Apply the equivalent of the sparse-checkout narrowing on the cursor path.

The verified `cursor-agent --help` surface above shows `-w/--worktree [name]`,
`--worktree-base <branch>`, and `--skip-worktree-setup` — and **no sparse
flag**. So the direct control does not exist. Two things follow, and you owe both:

1. Determine empirically whether the shim can narrow the tree anyway. The two
   candidate levers are (a) a `.cursor/worktrees.json` worktree setup script,
   which `--skip-worktree-setup` proves `cursor-agent` runs after creating the
   worktree — note `~/cursor-shim/.cursor/` does not exist today, so this is
   untested ground; and (b) applying `git sparse-checkout set` to the worktree
   from `cursor-spine` after `cursor-agent` has created it, if the shim gets
   control at a point where the path is known (`worktree_name_for()` at `:76`
   derives the name, so the path is derivable). Test what actually works. Do not
   reason about it — run it.
2. If neither lever works, record the limitation explicitly as a board finding
   with the **cited command and its output**, and state what the shim can do
   instead.

- **Done when:** either the cursor path produces a demonstrably sparse worktree
  (prove it with `find <wt> -type f -not -path '*/.git/*'`), or a board finding
  names precisely why it cannot, with a cited command and its output, plus the
  stated alternative.

### Ordering

3.3 -> 3.1 -> 2.3. Nothing here depends on your sibling's work; do not wait on
it.
