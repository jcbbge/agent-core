...
## Tasks (AGNT profiles — YOUR PARTITION ONLY)
1. In coordinator.md, orchestrator.md, coder.md, researcher.md: add standing pull-loop behavior + two stopping states only; deposit never deliver; cite COMMS-ARCH plane 5. Align wording with unit brief Part 1 — quote, do not paraphrase. — done when: `rg -n 'pull loop|two stopping|stopping state' primitives/profiles/{coordinator,orchestrator,coder,researcher}.md` hits all four.
2. In concierge.md: state the exception ONCE — may address panes (plane 4 OPERATOR DIRECTIVES); MUST leave board trace; not a licence for ranks 1–4 to message peers. — done when: exception present once with leave-a-trace.
3. Write .done at /Users/jrg/agent-core/briefs/tower/codify-stigmergy/workers/profiles.done with rg evidence + deviations.

## Constraints
- Touch ONLY: primitives/profiles/coordinator.md, orchestrator.md, coder.md, researcher.md, concierge.md. Do not commit.
- Do not edit COMMS-ARCH / control-flow / RESPONSIBLE-PARTY / brief / cursor-fleet (siblings own those).
- No mocks. Quote law verbatim when restating.

## Report back with
- Per-profile one-line change summary
