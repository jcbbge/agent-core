...
| Pantry utensils actually run | AGENTS.md §Utensils | `utensil-guard.mjs` (CC PreToolUse `Read\|Grep` + `Bash`; cursor preToolUse `Read\|Grep\|Shell`) + `utensil-guard-pi.ts` (pi/prime `tool_call`). Denies: 3k+ PHP/JS/TS/TSX Read → bigfile; NL Grep → colgrep; bare sleep / sleep-poll → latch; transcript-dir grep → pickbrain. Bypass: `UTENSIL_GUARD=off` | HOOK | CC + cursor + pi + prime. Leftover DOCTRINE: coraline callers/impact, composto IR (no safe native-tool matcher yet) |
...
| Spawn through the door (stamp role/name/task, verified submit) | `~/muster/docs/agent-spawn-sop.md`; control-flow.md §Naming | `~/muster/bin/muster-spawn` (DOOR; forwarder `~/bin/spine-spawn`) + spawn-door refusing raw `herdr agent start`: `spawn-door.sh` (CC PreToolUse + cursor preToolUse — deny schema verified against cursor.com/docs/agent/hooks 2026-08-14) + `spawn-door-pi.ts` (pi `tool_call` `{block:true}`; 5-shape parity test) | DOOR+HOOK | CC + pi + cursor |
| Workspace mutations announced (board trace + operator line) | session loop (d)/(e), 2026-08-14 | spawn-door refusing raw `herdr workspace close` (all three harnesses, as above); no live workspace-close door installed | HOOK+DOCTRINE | CC + pi + cursor |
| Close needs a reason: Done (proof path) or Parked (pickup path) | control-flow.md §Diagnosis ≠ Land | none mechanical — close law in control-flow.md / herdr skill | DOCTRINE | any harness |
| Rulings carry scope (where they apply AND where they do not) | 2026-08-14 overcorrection ping-pong | none mechanical — no live ruling door installed | DOCTRINE | any harness |
| Operator relay guarantee (`to:"operator"` mail blocks turn-end) | primitives/rules/comms-arch.md §Hard invariants (successor to COMMS-ARCH §Hard invariants) | none — `stop-guard.mjs` read an inbox-state helper in the old bus's now-deleted support library; both are gone, and no hook yet reads the new bus (`tower.mjs` / `tower` CLI) for a stop-blocking check | DOCTRINE | none — compilation candidate: a stop hook running `tower inbox operator` (unread = `id > acked_id`) for open `question` / unrelayed `deliverable` rows |
| Operator-question doorbell (`tower send --kind question` + `herdr notification show` same breath) | primitives/rules/comms-arch.md §Hard invariants; control-flow.md | `doorbell-cursor.sh` (cursor `stop` + `followup_message`) — but it shells out to the same now-deleted inbox-state helper (`primitives/hooks/doorbell-cursor.sh:31-32`); the hook is now broken by this cutover, not merely undocumented | DOCTRINE | cursor: broken, pending repoint (reported, not fixed here — out of this unit's partition). CC: no equivalent since `stop-guard.mjs` is gone. pi: DOCTRINE (unchanged) |
...
| Write integrity (append-only INSERT; `sender`/`body`/`kind` required; `dedup` dedupes retries) | primitives/rules/comms-arch.md §Board row schema (successor to COMMS-ARCH §Board row schema) | `tower.mjs` `send()` — parameterized INSERT under `sender NOT NULL` + `body NOT NULL` + `dedup UNIQUE`; the `tower` CLI is the only path to it | DOOR | all writers via the `tower` CLI. The old JSONL row-shape guarantee (flocked append, the old bus's board-write guard function, one-newline-terminated-JSON-object) has no successor question on a SQLite table — a row is a row, not a parsed line — so it is deleted outright, not translated |
| Alarm validation at emit + read (no empty questions) | primitives/rules/comms-arch.md §Alarm rationalization | none — the new bus's `send()` rejects only a NULL/absent `body` (SQL `NOT NULL`), not a blank/whitespace one; there is no `questionRejectReason()`, no dead-letter sink, and no read-time filter equivalent to `openQuestionRows()` | DOCTRINE | none — compilation candidate: a `send()`-side guard rejecting blank `kind=question` bodies, or a `tower inbox` filter mirroring the old dead-letter behavior |
...
| Reaping (done = gone; no trophy panes) | control-flow.md §Reaping | none mechanical | DOCTRINE | candidate: statem-gated close |
| Worktree lifecycle (unit-scoped birth/death, flat topology, sparse by default, safe teardown) | worktree-lifecycle.md | DOOR: `cursor-finish:460` `trap cleanup_and_preserve_rc EXIT` (cursor path) + `muster-spawn reap` on any worktree no live pane occupies (all paths, reconciled not evented) + `sparse-checkout set --no-cone` on both spawners | DOOR | all three — the reconciler watches worktree roots and pane occupancy, neither of which knows which engine is seated |
...
| Naming prefixes before agent start | control-flow.md §Naming | `muster-spawn` stamps; raw path refused by spawn-door (all three harnesses) | DOOR+HOOK | CC + pi + cursor |
