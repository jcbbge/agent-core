...
| Operator-question doorbell (`ask_user` + `herdr notification show` same breath) | COMMS-ARCH §Hard invariants; control-flow.md | `doorbell-cursor.sh` (cursor `stop` + `followup_message`) | HOOK | cursor. CC: `stop-guard.mjs` blocks stop on open questions but does not notify — doorbell DOCTRINE. pi: DOCTRINE |
...
| Worktree lifecycle (unit-scoped birth/death, flat topology, sparse by default, safe teardown) | worktree-lifecycle.md | DOOR: `cursor-finish:460` `trap cleanup_and_preserve_rc EXIT` (cursor path) + `handlers/18-worktree-reconcile` invoking `spine-spawn reap` on any worktree no live pane occupies (all paths, reconciled not evented) + `sparse-checkout set --no-cone` on both spawners | DOOR | all three — the reconciler watches worktree roots and pane occupancy, neither of which knows which engine is seated |
