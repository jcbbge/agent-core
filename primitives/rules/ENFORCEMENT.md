...
| Worktree lifecycle (unit-scoped birth/death, flat topology, sparse by default, safe teardown) | worktree-lifecycle.md | DOOR: `cursor-finish:460` `trap cleanup_and_preserve_rc EXIT` (teardown, cursor path) + `spine-spawn --sparse` / Touch-ONLY-derived sparse (both spawners). DOCTRINE: spine-side reap (`spine-spawn --help` registers `reap <path>`) exists but nothing invokes it automatically | DOOR+DOCTRINE | sparse: CC + cursor. teardown: cursor path DOOR; spine-side auto-invocation DOCTRINE |
