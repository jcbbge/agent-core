...
fn sessionIdFromPath(allocator: std.mem.Allocator, path: []const u8) ![]const u8 {
    ...
    if (std.mem.endsWith(u8, base, ".jsonl")) {
        const stem = base[0 .. base.len - ".jsonl".len];
        // pi filenames embed uuid after last underscore
        if (std.mem.lastIndexOfScalar(u8, stem, '_')) |idx| {
            return try allocator.dupe(u8, stem[idx + 1 ..]);
        }
...
/// Pair assistant toolCall name bash to toolResult.toolCallId.
...
pub fn extractFromTranscript(allocator: std.mem.Allocator, source_path: []const u8, project_key: []const u8) ![]Row {
    ...
    var session_cwd = try allocator.dupe(u8, project_key);
    ...
    while (try io_ctx.readLineInto(allocator, &reader, &line_buf)) {
        ...
        if (schema.getObjectField(root, schema.pi_paths.row_type)) |type_val| {
            if (schema.getString(type_val)) |typ| {
                if (std.mem.eql(u8, typ, "session")) {
                    if (schema.getObjectField(root, schema.pi_paths.session_cwd)) |cwd_val| {
                        if (schema.getString(cwd_val)) |cwd| {
                            allocator.free(session_cwd);
                            session_cwd = try allocator.dupe(u8, cwd);
                        }
                    }
                }
            }
        ...
        if (std.mem.eql(u8, role, "assistant")) {
            for (wrapped.items) |item| {
                ...
                if (!std.mem.eql(u8, typ, "toolCall")) continue;
                const name = if (obj.get("name")) |nv| schema.getString(nv) orelse "" else "";
                ...
                if (obj.get("arguments")) |args_val| {
                    if (args_val == .object) {
                        if (args_val.object.get("command")) |cmd_val| {
                ...
                if (command) |cmd| {
                    try calls.append(.{
                        ...
                        .cwd = try allocator.dupe(u8, session_cwd),
                    });
                }
            }
        } else if (std.mem.eql(u8, role, "toolResult")) {
            const tool_name = if (message.get("toolName")) |tn| schema.getString(tn) orelse "" else "";
            if (!std.mem.eql(u8, tool_name, "bash")) continue;
            const call_id = if (message.get("toolCallId")) |id_val| schema.getString(id_val) orelse "" else "";
            const result_text_raw = schema.textContent(content_val);
            const owned_text = try allocator.dupe(u8, result_text_raw);
            const details = message.get("details");
            const is_error = if (message.get("isError")) |e| schema.getBool(e) orelse false else false;
...
test "extract pi sample session" {
    io_ctx.ensureTestIo(std.testing.allocator);
    const path = "/Users/jrg/.pi/agent/sessions/--Users-jrg-agent-core--/2026-08-11T18-15-57-199Z_019ff209-d00f-7e0f-ad9a-9203e17710b4.jsonl";
    const file = io_ctx.openAbs(path) catch return;
    file.close(io_ctx.io());

    const rows = try extractFromTranscript(std.testing.allocator, path, "--Users-jrg-agent-core--");
    defer {
