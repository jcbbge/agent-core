...
```bash
cp -r ~/Documents/_agents/agent-core/harnesses/_template ~/Documents/_agents/agent-core/harnesses/[name]
```
...
deploy_mcp() {
  ...
  python3 ~/Documents/_agents/agent-core/deploy/patch-mcp.py \
...
```bash
~/Documents/_agents/agent-core/deploy.sh --dry-run [name]   # verify plan
~/Documents/_agents/agent-core/deploy.sh [name]              # execute
```
...
```bash
~/Documents/_agents/agent-core/observe/emit.sh [harness-name] hook session_start
```
